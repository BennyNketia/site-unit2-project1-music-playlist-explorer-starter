...
**Page Purpose:**
The Featured page displays a single randomly selected playlist with emphasis on visual presentation and song discovery. This page serves as an engaging entry point to the app, showcasing playlist content in a more detailed layout than the All Playlists view.

**Layout Structure:**

The Featured page uses a two-column split layout:
...
**Navigation Bar:**
- Fixed at top of page (matching All Playlists header)
- Two navigation links:
  - "Featured" (active state when on featured.html)
  - "All Playlists" (links to index.html)
- Same Spotify-inspired styling as filter buttons
- Positioned in header alongside or replacing filter buttons

**Visual Design Principles:**
- Dark theme consistent with existing app (#121212 background)
- Emphasis on the cover art as visual anchor
- Clear hierarchy: image → title → songs
- Generous whitespace for breathing room
- Smooth transitions when navigating between pages

#### Function Spec: `selectRandomPlaylist()`

**Purpose:** Select a random playlist from the loaded data to display on the Featured page
...
- Ensures different playlist on each page load/refresh
- Does NOT remember previous selections (truly random each time)
...
- Does NOT run on navigation back/forward (browser caches page state)
...
**Accessibility:**
- Announce playlist change to screen readers (aria-live region)
- Ensure random selection is perceivable by all users

#### Function Spec: `renderFeaturedPlaylist(playlist)`

**Purpose:** Display the selected playlist in the Featured page layout

**Input:**
- `playlist` (Playlist object) — The randomly selected playlist to display

**Output:**
- No return value (void function)
- Side effects: Updates multiple DOM elements with playlist data

**DOM Elements Updated:**
- `.featured-cover-image` — Set `src` to `playlist.coverImage`
- `.featured-playlist-title` — Set `textContent` to `playlist.title`
- `.featured-playlist-creator` — Set `textContent` to `playlist.creator`
- `.featured-playlist-stats` — Display song count (e.g., "12 songs")
- `.featured-like-button` — Set up click handler with `playlist.id`
- `.featured-like-count` — Display formatted like count
- `.featured-songs-list` — Populate with all songs (reuse `createSongElement()` from existing code)

**Behavior:**
- Should be called after `selectRandomPlaylist()` returns a playlist
- Should clear any previous content before rendering
- Should attach all event listeners (like buttons, song interactions)
- Should maintain consistency with modal song display from All Playlists page

**Error Handling:**
- If playlist is null/undefined, show error message
- If playlist.songs is empty, show "No songs in this playlist yet"
- Handle image loading errors with gradient fallback
...
- Visual active state uses green highlight (#1DB954) like filter buttons

**User Experience:**
- Clicking "Featured" from All Playlists page loads featured.html
- Clicking "All Playlists" from Featured page loads index.html
- Each load of featured.html selects a NEW random playlist
- Users can refresh featured.html to see a different random playlist
- Navigation preserves scroll position on All Playlists page (browser default)
...
1. **Separate HTML file** (featured.html) — Keeps concerns separated, easier to maintain distinct layouts
2. **Shared CSS file** (style.css) — Maintains design consistency across pages
3. **Shared data loading logic** — Both pages load from data/data.json
4. **Reusable functions** — `createSongElement()`, `formatLikeCount()`, etc. can be reused
5. **Random selection on each load** — Provides fresh experience, no need for "refresh" button
...
├── script.js           # JavaScript for All Playlists
├── featured.js         # JavaScript for Featured page (NEW)
├── style.css           # Shared styles (updated with Featured page styles)
└── data/
    └── data.json       # Playlist data
```

**Alternative Approach (Optional Note):**
The Featured page could be set as the home page by:
1. Renaming `featured.html` to `index.html`
2. Renaming current `index.html` to `playlists.html`
3. Updating all navigation links accordingly
This would make the Featured experience the first thing users see.
